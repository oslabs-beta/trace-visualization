...
statementController.populateSelectCols = (
  ...
  if (stmt.fromClause[0].JoinExpr) {
    if (
      stmt.fromClause[0].JoinExpr.jointype === "JOIN_LEFT" ||
      stmt.fromClause[0].JoinExpr.jointype === "JOIN_RIGHT"
    ) {
      //check to see if alias needs to be populated
      if (stmt.fromClause[0].JoinExpr.larg.RangeVar.alias) {
        aliasObj[stmt.fromClause[0].JoinExpr.larg.RangeVar.alias.aliasname] =
          stmt.fromClause[0].JoinExpr.larg.RangeVar.relname;
        aliasObj[stmt.fromClause[0].JoinExpr.rarg.RangeVar.alias.aliasname] =
          stmt.fromClause[0].JoinExpr.rarg.RangeVar.relname;
      }
    ...
    //inner join in used
      ...
      for (let i = 0; i < colsArr.length; i++) {
        columns[stmt.fromClause[0].JoinExpr.larg.RangeVar.relname].push(
          colsArr[i].ResTarget.val.ColumnRef.fields[0].String.str
        );
      }
    }
  }
  //join statement not used
    ...
    if (stmt.fromClause[0].RangeVar.alias) {
      aliasObj[stmt.fromClause[0].RangeVar.alias.aliasname] =
        stmt.fromClause[0].RangeVar.relname;
    }
    ...
    for (let i = 0; i < colsArr.length; i++) {
      columns[stmt.fromClause[0].RangeVar.relname].push(
        colsArr[i].ResTarget.val.ColumnRef.fields[0].String.str
      );
    }
  }
